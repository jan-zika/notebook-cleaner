...
def extract_widget_labels(nb):
    ...
    try:
        ...
        for model_id, entry in widget_meta.items():
            ...
            desc = state.get("description") or entry.get("model_name") or "Unnamed widget"
            labels[model_id] = desc
    except Exception:
        pass
    return labels
        ...
        if "outputs" in cell:
            ...
            for out in cell["outputs"]:
                if (
                    ...
                    removed_ids.append(widget_labels.get(model_id, f"Widget {model_id}"))
                    changed = True
                    # skip widget output
                else:
            ...
            if removed_ids:
                list_text = "\n".join(f"- {label}" for label in removed_ids)
                placeholder_md = (
